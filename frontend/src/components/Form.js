...
// Selectable Countries, current Country selection and change handler get passed to form as props

function Form(props) {
  // Creates options dropdowns based on country that is passed in
  function createSelections(countries) {
    if (countries) {
      return countries.map((country) => {
        return (
          <option className='option' value={country}>
            {country}
          </option>
        );
      });
  ...
  // Each select elements value gets updated by country selection state, which is itself manipulated by change handler function (controlled component). Arrow span is added to enable styling of selection arrow.
  return (
    <form className='form'>
